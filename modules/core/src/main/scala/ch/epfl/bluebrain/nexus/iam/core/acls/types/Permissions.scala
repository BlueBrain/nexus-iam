...
/**
  ...
  *
  * @param set the set of [[ch.epfl.bluebrain.nexus.iam.core.acls.types.Permission]]s
  */
final case class Permissions(set: Set[Permission]) extends (Permission => Boolean) {
  ...
  /**
    * Checks whether all the __Permissions__ of the argument are included in this set.
    *
    * @param permissions the perm issions to check for inclusion in this set
    * @return __true__ if all the argument permissions are included, __false__ otherwise
    */
...
object Permissions {
  ...
  /**
    * Convenience factory method to construct a [[ch.epfl.bluebrain.nexus.iam.core.acls.types.Permissions]] instance from var args.
    *
